...
"""Build unicontainer._core, a Cython extension over the UniContainer C ABI.
...
PKG_DIR = os.path.join(HERE, "unicontainer")
VENDOR_DIR = os.path.join(HERE, "_nimsrc")
NIMBLE_FILE = "UniContainer.nimble"
...
if sys.platform == "win32":
    LIB_NAME, BUNDLED = "UniContainer.lib", False
    LINK_ARGS, NIMBLE_TASK = [], "clibMsvc"
elif sys.platform == "darwin":
    LIB_NAME, BUNDLED = "libUniContainer.dylib", True
    LINK_ARGS, NIMBLE_TASK = ["-Wl,-rpath,@loader_path"], "clib"
else:
    LIB_NAME, BUNDLED = "libUniContainer.so", True
...
def nim_project_dir():
    """Where UniContainer.nimble lives: the real repo root in a normal
...
def ensure_lib_built():
    ...
    try:
        ...
        raise SystemExit(
            "setup.py: `nimble` not found on PATH. Building unicontainer from "
...
# The sdist ships the pre-transpiled unicontainer/_core.c, not the .pyx (Cython
...
# needing no Cython.
pyx = os.path.join("unicontainer", "_core.pyx")
ext = Extension(
    "unicontainer._core",
    sources=[pyx if os.path.exists(os.path.join(HERE, pyx)) else os.path.join("unicontainer", "_core.c")],
    include_dirs=[INCLUDE],
    library_dirs=[LIB_DIR],
    extra_link_args=LINK_ARGS,
    libraries=["UniContainer"],
)
...
setup(
    ...
    package_data={"unicontainer": [LIB_NAME] if BUNDLED else []},
    exclude_package_data={"unicontainer": ["_core.c"]},
    zip_safe=False,
)
