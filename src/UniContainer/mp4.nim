...
## Writing an MP4: tracks declared up front, samples appended, the tables
## assembled at close.
##
## The box layer beside this one says how a box is shaped; this says which
## boxes an ISO base media file needs and in what order. Neither reads a
## sample: a codec configuration is copied from the encoder unexamined, which
## is what keeps this a muxer rather than half a decoder.
##
## Two writers, because two shapes of file are wanted. `Mp4Writer` holds the
## sample tables in memory and writes them at `close`, which is what a file on
## disk wants. `FragmentedMp4Writer` emits a fragment at a time, which is what
## a stream wants and what lets a reader start before the writer has finished.
...
type
  ContainerError* = object of CatchableError
    ## A container this library cannot write, or a track whose declared shape
    ## contradicts the samples handed to it.

  TrackKind* = enum
    ## What a track carries. `tkOther` covers subtitles, timecode and data
    ## tracks: they are written so a caller can carry a whole file through, and
    ## skipped by anything asking for pictures or sound.
    tkVideo = "video"
    tkAudio = "audio"
    tkOther = "other"
  ...
  body:
    for track in tracks:
      if track.kind == tkOther:
        raise newException(ContainerError, "mp4: a track must be video or audio")
      if track.codec.len != 4:
        raise newException(ContainerError,
          "mp4: a codec is four characters, not '" & track.codec & "'")
      if track.timescale notin 1 .. 1_000_000_000:
        raise newException(ContainerError, "mp4: implausible timescale")
      if track.kind == tkVideo and
          (track.width notin 1 .. MaxDimension or
           track.height notin 1 .. MaxDimension):
        raise newException(ContainerError, "mp4: a video track needs its size")
      if track.edits.len > MaxWriterEdits:
        raise newException(ContainerError, "mp4: implausible edit list")
      for edit in track.edits:
        ...
        if edit.duration notin 0'i64 .. high(uint32).int64:
          raise newException(ContainerError, "mp4: an edit duration is out of range")
        if edit.mediaTime < -1 or edit.mediaTime > high(int32).int64:
          raise newException(ContainerError, "mp4: an edit media time is out of range")
...
proc writeSample*(writer: var Mp4Writer; track: int; data: openArray[byte];
  ...
  require:
    ...
    duration >= 0
  body:
    if writer.closed: raise newException(ContainerError, "mp4: writer is closed")
    if track >= writer.params.len:
      raise newException(ContainerError, "mp4: track index past the file")
    if writer.samples[track].len >= MaxWriterSamples:
      raise newException(ContainerError, "mp4: too many samples for one track")
    if data.len == 0:
      raise newException(ContainerError, "mp4: an empty sample has no meaning")
...
proc close*(writer: var Mp4Writer) {.contractual.} =
  ...
  body:
    ...
    if empty:
      ...
      raise newException(ContainerError, "mp4: no sample was written")
...
proc writeSample*(writer: var FragmentedMp4Writer; track: int;
  ...
  body:
    ...
    if writer.pending[track].len >= MaxWriterSamples:
      raise newException(ContainerError, "mp4: too many samples in one fragment")
    if data.len == 0:
      raise newException(ContainerError, "mp4: an empty sample has no meaning")
